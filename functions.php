...
Class RoadThemes {
	...
	*/
	...
	function road_search_form( $form ) {
		if(get_search_query()!=''){
			...
			$search_str = esc_html__( 'Search...', 'allen' );
		}
		...
		$form .= 'jQuery(document).ready(function(){
			jQuery("#search_input").focus(function(){
				if(jQuery(this).val()=="'.esc_html__( 'Search...', 'allen' ).'"){
					jQuery(this).val("");
				}
			});
			jQuery("#search_input").focusout(function(){
				if(jQuery(this).val()==""){
					jQuery(this).val("'.esc_html__( 'Search...', 'allen' ).'");
				}
			});
			jQuery("#blogsearchsubmit").click(function(){
				if(jQuery("#search_input").val()=="'.esc_html__( 'Search...', 'allen' ).'" || jQuery("#search_input").val()==""){
	...
	function road_woo_search_form( $form ) {
		...
		if(get_search_query()!=''){
			...
			$search_str = esc_html__( 'Search product...', 'allen' );
		}
		...
		$form .= 'jQuery(document).ready(function(){
			jQuery("#ws").focus(function(){
				if(jQuery(this).val()=="'.esc_html__( 'Search product...', 'allen' ).'"){
					jQuery(this).val("");
				}
			});
			jQuery("#ws").focusout(function(){
				if(jQuery(this).val()==""){
					jQuery(this).val("'.esc_html__( 'Search product...', 'allen' ).'");
				}
			});
			jQuery("#wsearchsubmit").click(function(){
				if(jQuery("#ws").val()=="'.esc_html__( 'Search product...', 'allen' ).'" || jQuery("#ws").val()==""){
					jQuery("#ws").focus();
					return false;
				}
			});
	...
	*/
	...
	function road_setup() {
		 ...
		 * to change 'allen' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'allen', get_template_directory() . '/languages' );
		...
		// Register menus
		register_nav_menu( 'primary', esc_html__( 'Primary Menu', 'allen' ) );
		register_nav_menu( 'topmenu', esc_html__( 'Top Menu', 'allen' ) );
		register_nav_menu( 'mobilemenu', esc_html__( 'Mobile Menu', 'allen' ) );
	...
	function road_scripts_styles() {
		...
		*/
		...
		if($road_opt['enable_less']){
			...
			switch ($presetopt) {
				case 2:
					$themevariables['primary_color'] = '#5bb300';
					$themevariables['sale_color'] = '#5bb300';
					$themevariables['menu_color'] = '#444';
				break;
				case 3:
					$themevariables['primary_color'] = '#ffa200';
					$themevariables['sale_color'] = '#ffa200';
					$themevariables['menu_color'] = '#444';
				break;
				case 4:
					$themevariables['primary_color'] = '#5f87d1';
					$themevariables['sale_color'] = '#5f87d1';
					$themevariables['menu_color'] = '#444';
				break;
				case 5:
					$themevariables['primary_color'] = '#3fb4fa';
					$themevariables['sale_color'] = '#3fb4fa';
					$themevariables['menu_color'] = '#444';
				break;
				case 6:
					$themevariables['primary_color'] = '#d8c2a8';
					$themevariables['sale_color'] = '#d8c2a8';
					$themevariables['menu_color'] = '#444';
				break;
			}
		...
		wp_enqueue_script( 'styleswitcher-js', get_template_directory_uri() . '/js/styleswitcher.js', array('scrollbar-js'), '20140826', true );
	...
	function road_widgets_init() {
		register_sidebar( array(
			'name' => esc_html__( 'Blog Sidebar', 'allen' ),
			'id' => 'sidebar-1',
			'description' => esc_html__( 'Sidebar on blog page', 'allen' ),
		...
		register_sidebar( array(
			'name' => esc_html__( 'Shop Sidebar', 'allen' ),
			'id' => 'sidebar-shop',
			'description' => esc_html__( 'Sidebar on shop page (only sidebar shop layout)', 'allen' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title"><span>',
			'after_title' => '</span></h3>',
		...
		register_sidebar( array(
			'name' => esc_html__( 'Category Sidebar', 'allen' ),
			'id' => 'sidebar-category',
			'description' => esc_html__( 'Small sidebar on product category page (only full width shop layout)', 'allen' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title"><span>',
			'after_title' => '</span></h3>',
		...
		register_sidebar( array(
			'name' => esc_html__( 'Category Filter', 'allen' ),
			'id' => 'sidebar-filter',
			'description' => esc_html__( 'Filter area on product category page (only full width shop layout)', 'allen' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title"><span>',
			'after_title' => '</span></h3>',
		...
		register_sidebar( array(
			'name' => esc_html__( 'Pages Sidebar', 'allen' ),
			'id' => 'sidebar-page',
			'description' => esc_html__( 'Sidebar on content pages', 'allen' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title"><span>',
			'after_title' => '</span></h3>',
	...
	function road_meta_box_callback( $post ) {
		...
		esc_html_e( 'This content will be used to replace the featured image, use shortcode here', 'allen' );
		echo '</label><br />';
	...
	function road_add_meta_box() {
		...
		foreach ( $screens as $screen ) {
			...
			add_meta_box(
				...
				esc_html__( 'Post featured content', 'allen' ),
	...
	*/
	...
	function road_get_font_url() {
		 ...
		 */
		if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'allen' ) ) {
			$subsets = 'latin,latin-ext';

			/* translators: To add an additional Open Sans character subset specific to your language,
			 * translate this to 'greek', 'cyrillic' or 'vietnamese'. Do not translate into your own language.
			 */
			$subset = _x( 'no-subset', 'Open Sans font: add new subset (greek, cyrillic, vietnamese)', 'allen' );
	...
	static function road_content_nav( $html_id ) {
		...
		if ( $wp_query->max_num_pages > 1 ) : ?>
			<nav id="<?php echo esc_attr($html_id); ?>" class="navigation" role="navigation">
				<h3 class="assistive-text"><?php esc_html_e( 'Post navigation', 'allen' ); ?></h3>
				<div class="nav-previous"><?php next_posts_link( esc_html__( '<span class="meta-nav">&larr;</span> Older posts', 'allen' ) ); ?></div>
				<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts <span class="meta-nav">&rarr;</span>', 'allen' ) ); ?></div>
			</nav>
		<?php endif;
	}
	/* Pagination */
	static function road_pagination() {
		...
		echo paginate_links( array(
			...
			'prev_text'    => esc_html__('Previous', 'allen'),
			'next_text'    =>esc_html__('Next', 'allen'),
		) );
	}
	/**
	...
	static function road_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'allen' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'allen' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
					...
					<header class="comment-meta comment-author vcard">
						<?php
							...
							printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
								...
								( $comment->user_id === $post->post_author ) ? '<span>' . esc_html__( 'Post author', 'allen' ) . '</span>' : ''
							);
							printf( '<time datetime="%1$s">%2$s</time>',
								get_comment_time( 'c' ),
								/* translators: 1: date, 2: time */
								sprintf( esc_html__( '%1$s at %2$s', 'allen' ), get_comment_date(), get_comment_time() )
							);
						?>
						<div class="reply">
							<?php comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'allen' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
						</div><!-- .reply -->
					</header><!-- .comment-meta -->
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'allen' ); ?></p>
					<?php endif; ?>

					<section class="comment-content comment">
						<?php comment_text(); ?>
						<?php edit_comment_link( esc_html__( 'Edit', 'allen' ), '<p class="edit-link">', '</p>' ); ?>
	...
	static function road_entry_meta() {
		...
		$tag_list = get_the_tag_list( '', esc_html__( ', ', 'allen' ) );

		$num_comments = (int)get_comments_number();
		$write_comments = '';
		if ( comments_open() ) {
			if ( $num_comments == 0 ) {
				$comments = esc_html__('0 comments', 'allen');
			} elseif ( $num_comments > 1 ) {
				$comments = $num_comments . esc_html__(' comments', 'allen');
			} else {
				$comments = esc_html__('1 comment', 'allen');
			}
			$write_comments = '<a href="' . get_comments_link() .'">'. $comments.'</a>';
		}

		$utility_text = esc_html__( '%1$s / Tags: %2$s', 'allen' );
	...
	static function road_entry_meta_small() {
		...
		// Translators: used between list items, there is a space after the comma.
		$categories_list = get_the_category_list( esc_html__( ', ', 'allen' ) );

		$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'allen' ), get_the_author() ) ),
			get_the_author()
		);
		
		$utility_text = esc_html__( 'Posted by %1$s / %2$s', 'allen' );
	...
	static function road_entry_comments() {
		...
		if ( comments_open() ) {
			if ( $num_comments == 0 ) {
				$comments = esc_html__('<span>0</span> comments', 'allen');
			} elseif ( $num_comments > 1 ) {
				$comments = '<span>'.$num_comments .'</span>'. esc_html__(' comments', 'allen');
			} else {
				$comments = esc_html__('<span>1</span> comment', 'allen');
			}
			$write_comments = '<a href="' . get_comments_link() .'">'. $comments.'</a>';
		}
		
		$utility_text = esc_html__( '%1$s', 'allen' );
	...
	*/
	function road_register_required_plugins() {
		...
		$config = array(
			...
			'strings'      => array(
				'page_title'                      => esc_html__( 'Install Required Plugins', 'allen' ),
				'menu_title'                      => esc_html__( 'Install Plugins', 'allen' ),
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'allen' ), // %s = plugin name.
				'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'allen' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'allen' ), // %1$s = plugin name(s).
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'allen' ), // %1$s = plugin name(s).
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'allen' ), // %1$s = plugin name(s).
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'allen' ), // %1$s = plugin name(s).
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'allen' ), // %1$s = plugin name(s).
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'allen' ), // %1$s = plugin name(s).
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'allen' ), // %1$s = plugin name(s).
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'allen' ), // %1$s = plugin name(s).
				'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'allen' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'allen' ),
				'return'                          => esc_html__( 'Return to Required Plugins Installer', 'allen' ),
				'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'allen' ),
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'allen' ), // %s = dashboard link.
